# ===========================================================================
#      http://www.gnu.org/software/autoconf-archive/ax_boost_mpi.html
# ===========================================================================
#
# SYNOPSIS
#
...
# DESCRIPTION
#
#   Test for MPI library from the Boost C++ libraries. The macro requires
...
[
	...
	if test "x$want_boost" = "xyes"; then
		...
		CPPFLAGS="$CPPFLAGS $BOOST_CPPFLAGS $MPI_CPPFLAGS"
		export CPPFLAGS

		LDFLAGS_SAVED="$LDFLAGS"
		LDFLAGS="$LDFLAGS $BOOST_LDFLAGS $MPI_LDFLAGS"
		export LDFLAGS
        ...
        [AC_LANG_PUSH([C++])
			 AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[@%:@include <boost/mpi.hpp>
                                   ...
                                   [[boost::mpi::environment env; return 0;]])],
		...
		if test "x$ax_cv_boost_mpi" = "xyes"; then
            ...
            if test "x$ax_boost_user_mpi_lib" = "x"; then
                for libextension in `ls $BOOSTLIBDIR/libboost_mpi*.so* $BOOSTLIBDIR/libboost_mpi*.dylib* $BOOSTLIBDIR/libboost_mpi*.a* 2>/dev/null | sed 's,.*/,,' | sed -e 's;^lib\(boost_mpi.*\)\.so.*$;\1;' -e 's;^lib\(boost_mpi.*\)\.dylib.*;\1;' -e 's;^lib\(boost_mpi.*\)\.a.*$;\1;'` ; do
                     ax_lib=${libextension}
                ...
                for libextension in `ls $BOOSTLIBDIR/boost_mpi*.dll* $BOOSTLIBDIR/boost_mpi*.a* 2>/dev/null | sed 's,.*/,,' | sed -e 's;^\(boost_mpi.*\)\.dll.*$;\1;' -e 's;^\(boost_mpi.*\)\.a.*$;\1;'` ; do
                     ax_lib=${libextension}
				    AC_CHECK_LIB($ax_lib, exit,
                                 [BOOST_MPI_LIB="-l$ax_lib"; AC_SUBST(BOOST_MPI_LIB) link_mpi="yes"; break],
                                 [link_mpi="no"])
            ...
            if test "x$ax_lib" = "x"; then
                AC_MSG_ERROR(Could not find a version of the Boost::MPI library!)
            fi
			if test "x$link_mpi" != "xyes"; then
