...
#ifndef PENNESHEATEQNFDM_RADIATION_HPP_
#define PENNESHEATEQNFDM_RADIATION_HPP_

#include "ScaFES.hpp"
#include "PennesBioheatEqnFDM.hpp"

template<typename CT, std::size_t DIM, typename Class>
class Radiation : public PennesBioheatEqnFDM<CT,DIM, Radiation<CT,DIM, Class> > {
  ...
  public:
     ...
     */
    Radiation(ScaFES::Parameters const& params,
        ...
        : PennesBioheatEqnFDM<CT, DIM, Radiation<CT,DIM,Class> >(params, gg, useLeapfrog,
    ...
    void updateBorder(std::vector<ScaFES::DataField<TT,DIM>>& vNew,
        ...
        /* Otherwise use Radiation boundary condition. */
        if (useRadiation == true) {
            vNew[0](idxNode) = vOld[0](idxNode);
            for (std::size_t pp = 0; pp < DIM; ++pp) {
                /* Radiation boundary condition has to be used in this direction. */
                if (idxNode.elem(pp) == (this->nNodes(pp)-1)) {
                    vNew[0](idxNode) += this->tau() * this->COEFF_A * (
                                        ...
                                        - ((2.0*this->gridsize(pp)/this->LAMBDA)
                                           * this->EPSILON * this->SIGMA * vOld[0](idxNode) * vOld[0](idxNode) * vOld[0](idxNode) * vOld[0](idxNode))
                                        + ((2.0*this->gridsize(pp)/this->LAMBDA)
