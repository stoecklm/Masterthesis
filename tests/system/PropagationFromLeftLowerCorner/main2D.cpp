...
#include "ScaFES.hpp"
#include "PropagationFromLeftLowerCorner.hpp"
...
int main(int argc, char *argv[]) {
    ...
    for (int ii = 0; ii < N_DATAFIELDS; ++ii) {
        writeToFile[ii] = ScaFES::WriteHowOften::LIKE_GIVEN_AT_CL;
    }
    std::vector<bool> computeError(N_DATAFIELDS, true);

    std::vector<double> geomparamsInit;

    PropagationFromLeftLowerCorner<double, DIM> ppp(paramsCl, gg, false,
