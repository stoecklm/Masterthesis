...
#ifndef HEATEQNFDM_DIRICHLET_TIMELINSPACECONST_HPP
#define HEATEQNFDM_DIRICHLET_TIMELINSPACECONST_HPP

#include "ScaFES.hpp"
#include "Neumann.hpp"

template<typename CT, std::size_t DIM>
class TimeLinSpaceConst : public Neumann<CT,DIM, TimeLinSpaceConst<CT,DIM> > {
  ...
  public:
     ...
     */
    TimeLinSpaceConst(ScaFES::Parameters const& params,
        ...
        : Neumann<CT, DIM, TimeLinSpaceConst<CT,DIM> >(params, gg, useLeapfrog,
    ...
    void evalInner(std::vector< ScaFES::DataField<CT, DIM> >& vNew,
        ...
        /* Vector for f. */
        vNew[0](idxNode) = this->RHO * this->C * 1.0; // - LAMBDA * 0
        /* Vector for g. */
        vNew[1](idxNode) = 0.0;
        /* Vector for y. */
    ...
    void evalBorder(std::vector< ScaFES::DataField<CT, DIM> >& vNew,
        ...
        if (useNeumann == true) {
            vNew[1](idxNode) = -1.0 * this->LAMBDA * 0.0;
        }
        /* Vector for y. */
        vNew[2](idxNode) = (1.0 + t);
    }
