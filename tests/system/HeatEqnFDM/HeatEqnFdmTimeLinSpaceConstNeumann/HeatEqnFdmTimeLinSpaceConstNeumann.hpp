...
class HeatEqnFdmTimeLinSpaceConstNeumann : public HeatEqnFDM<CT,DIM, HeatEqnFdmTimeLinSpaceConstNeumann<CT,DIM> > {
  ...
  public:
     ...
     */
    HeatEqnFdmTimeLinSpaceConstNeumann(ScaFES::Parameters const& params,
        ...
        : HeatEqnFDM<CT, DIM, HeatEqnFdmTimeLinSpaceConstNeumann<CT,DIM> >(params, gg, useLeapfrog,
    ...
    void evalBorder(std::vector< ScaFES::DataField<CT, DIM> >& vNew,
        ...
        if (useNeumann == true) {
            vNew[1](idxNode) = -1.0 * this->LAMBDA
                               * timeLinSpaceConstdSpace1stOrder<CT,DIM>(x, t, 0);
        }
        /* Vector for y. */
        vNew[2](idxNode) = timeLinSpaceConstFunc<CT,DIM>(x, t);
    }
    ...
    void updateBorder(std::vector<ScaFES::DataField<TT,DIM>>& vNew,
        ...
        if (useNeumann == true) {
            ...
            for (std::size_t pp = 0; pp < DIM; ++pp) {
                /* Neumann boundary condition has to be used in this direction. */
